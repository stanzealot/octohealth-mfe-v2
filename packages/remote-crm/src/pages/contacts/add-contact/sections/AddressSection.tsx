...
export function AddressSection({ handler, defaultCountry, defaultState }: AddressSectionProps) {
  const {
    register,
    watch,
    setValue,
  ...
  const loadStateOptions = useCallback(makeLoadStateOptions(countryId), [countryId]);

  const defaultAddressType = useMemo(
    () =>
      addressType?.value
        ? ADDRESS_TYPE_OPTIONS.find((o) => o.value === addressType.value)
        : undefined,
    [addressType],
  );
      ...
      <Flex gap="3rem" flexDir={{ base: 'column', md: 'row' }}>
        <Box flex={1}>
          <AppAsyncSelect
            ...
            value={
              countryId ? { label: defaultCountry?.label ?? countryId, value: countryId } : null
            }
            onChange={(opt) => {
        ...
        <Box flex={1}>
          <AppAsyncSelect
            ...
            onChange={(opt) => setValue('stateId', (opt as StrOption | null)?.value ?? '')}
