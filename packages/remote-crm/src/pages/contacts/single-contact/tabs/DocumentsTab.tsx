...
import { DOCUMENT_STATUS_COLORS, DOCUMENT_CATEGORY_COLORS } from './shared/status-colors';
import { DocIcon } from './utils/doc-icon';
  ...
  const columns: TableColumn<Document>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        minWidth: '13rem',
        cell: (r) => (
          <Badge colorPalette={DOCUMENT_CATEGORY_COLORS[r.category]} size="sm">
            {r.category}
          </Badge>
        ),
      },
      ...
      { name: 'Uploaded Date', selector: (r) => r.uploadedDate, sortable: true, minWidth: '14rem' },
      {
        ...
        minWidth: '12rem',
        cell: (r) => (
          <Badge colorPalette={DOCUMENT_STATUS_COLORS[r.status]} size="sm">
            {r.status}
          </Badge>
        ),
      },
        ...
        cell: (row) => (
          <ActionMenu
            actions={[
              ...
              {
                label: 'Delete',
                cta: () => console.log('Delete document', row.id),
                allowPopover: true,
                confirmationText: 'Delete this document?',
              },
            ]}
          />
        ),
