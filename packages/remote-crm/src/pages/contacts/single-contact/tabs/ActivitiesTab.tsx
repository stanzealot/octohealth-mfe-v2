...
import { ACTIVITY_STATUS_COLORS, ACTIVITY_PRIORITY_COLORS } from './shared/status-colors';

const StatusBadge = memo(({ status }: { status: Activity['status'] }) => (
  <Badge colorPalette={ACTIVITY_STATUS_COLORS[status]} size="sm">
    {status}
  </Badge>
));
StatusBadge.displayName = 'ActivityStatusBadge';

const PriorityBadge = memo(({ priority }: { priority: Activity['priority'] }) => (
  <Badge colorPalette={ACTIVITY_PRIORITY_COLORS[priority]} size="sm">
    {priority}
  </Badge>
));
PriorityBadge.displayName = 'ActivityPriorityBadge';
        ...
        cell: (row) => (
          <ActionMenu
            actions={[
              ...
              {
                label: 'Delete',
                cta: () => console.log('Delete activity', row.id),
                allowPopover: true,
                confirmationText: 'Delete this activity?',
              },
            ]}
          />
        ),
