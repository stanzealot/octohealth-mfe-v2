/**
 * AddressesTab — shows the entity's addresses in a searchable table
 * with Add Address modal (matches monolith addresses tab exactly)
 */
              ...
              >
                ...
                <Flex gap="1.2rem" flexDir={{ base: 'column', md: 'row' }}>
                  <AppSelect
                    ...
                    onChange={(opt) => {
                      const next = [...rows];
                      next[i] = { ...next[i], country: (opt as any)?.value ?? '' };
                      setRows(next);
                    }}
                    height="4.8rem"
                  />
                  <AppSelect
                    ...
                    onChange={(opt) => {
                      const next = [...rows];
                      next[i] = { ...next[i], state: (opt as any)?.value ?? '' };
                      setRows(next);
                    }}
                    height="4.8rem"
                  />
                  <AppSelect
                    ...
                    onChange={(opt) => {
                      const next = [...rows];
                      next[i] = { ...next[i], city: (opt as any)?.value ?? '' };
                      setRows(next);
                    }}
                    height="4.8rem"
                  />
