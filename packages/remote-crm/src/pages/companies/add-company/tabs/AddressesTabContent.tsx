              ...
              >
                ...
                <Box mb="1.2rem">
                  <AppInput
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                      const n = [...rows];
                      n[i] = { ...n[i], address1: e.target.value };
                      setRows(n);
                    }}
                  />
                </Box>
                <Flex gap="1.2rem" flexDir={{ base: 'column', md: 'row' }}>
                  <AppSelect
                    ...
                    onChange={(opt) => {
                      const n = [...rows];
                      n[i] = { ...n[i], country: (opt as { value: string } | null)?.value ?? '' };
                      setRows(n);
                    }}
                    height="4.8rem"
                  />
                  <AppSelect
                    ...
                    onChange={(opt) => {
                      const n = [...rows];
                      n[i] = { ...n[i], state: (opt as { value: string } | null)?.value ?? '' };
                      setRows(n);
                    }}
                    height="4.8rem"
                  />
                  <AppSelect
                    ...
                    onChange={(opt) => {
                      const n = [...rows];
                      n[i] = { ...n[i], city: (opt as { value: string } | null)?.value ?? '' };
                      setRows(n);
                    }}
                    height="4.8rem"
                  />
...
function AddressesTabContentBase() {
  ...
  const filtered = useMemo(() => {
    ...
    return addresses.filter(
      (a) => a.street.toLowerCase().includes(q) || a.country.toLowerCase().includes(q),
    );
  }, [addresses, searchTerm]);

  const handleAdd = useCallback(
    (a: Omit<Address, 'id'>) => setAddresses((p) => [...p, { ...a, id: Date.now().toString() }]),
    [],
  );
  const handleDelete = useCallback(
    (id: string) => setAddresses((p) => p.filter((a) => a.id !== id)),
    [],
  );
